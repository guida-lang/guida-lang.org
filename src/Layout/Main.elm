...
view config session toSessionMsg children =
    ...
    , Html.node "dialog"
        ...
        [ Html.div [ Attr.class "fixed inset-0 top-14 bg-zinc-400/20 backdrop-blur-xs data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in dark:bg-black/40" ] []
        , Header.view
            { session = session
